...
cd /var/lib/jenkins/helm/flaskdb
...
# Rename the package
mv /tmp/flaskdb-$NEW_VERSION.tgz /tmp/helm-project-$NEW_VERSION.tgz

# Push the package to the GCP bucket
gsutil cp /tmp/helm-project-$NEW_VERSION.tgz gs://flask_helm/

# Cleanup
rm /tmp/helm-project-$NEW_VERSION.tgz

echo "Chart helm-project-$NEW_VERSION.tgz has been uploaded to the flask_helm bucket."

# List all versions, sorted
ALL_VERSIONS=$(gsutil ls gs://flask_helm/ | grep helm-project | sort)
